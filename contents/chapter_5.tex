...
\begin{itemize}
  \item \textbf{Q1：}基于在线模拟器预测的调度策略，相比简单指标调度能否改善端到端 TTFT（尤其尾延迟）与吞吐？
  \item \textbf{Q2：}预测模块的吞吐与延迟是否满足在线调度需求？是否会成为调度关键路径瓶颈？
  \item \textbf{Q3：}同步闭环与实现优化（pattern-level、prediction reuse）在系统中是否必要？它们带来怎样的收益？
  \item \textbf{Q4（可选）：}跨预测表/跨模型查表时，预测信号的排序一致性如何？为何 mean 变化小但 tail 更敏感？
\end{itemize}
